...
public final class FetchPlanner {
    ...
    public Flux<ObjectResult> fetch(BatchRequest request) {
        ...
        var inputs = request.getObjectKeys().stream().map(FetchInput::newInput).toList();
    ...
    private Stream<Path> resolveSourcePaths(
        ...
        if (propertyMappingOptional.isPresent()) {
            return propertyMappingOptional.stream()
                    .flatMap(propertyMapping ->
                            resolveSourcePaths(sourceTypeRef, propertyMapping, selectedProperty, basePath));
        }
        ...
        if (selectedProperty.getProperty() instanceof AbstractRelation relation) {
            ...
            return modelMapping.getObjectTypeMappings(relTargetType).stream()
                    .filter(targetTypeMapping ->
                            targetTypeMapping.getSourceRoot().equals(typeMapping.getSourceRoot()))
                    .flatMap(targetTypeMapping ->
                            resolveSourcePaths(selectedProperty.getNestedRequest(), targetTypeMapping, basePath)
                                    .stream());
        }
    ...
    private Stream<Path> resolveSourcePaths(
            ...
            SelectedProperty selectedProperty,
            Path basePath) {
        var sourcePaths = propertyMapping.getPathMappings().stream().flatMap(pathMapping -> {
            // TODO: Conditionality & recursion
            var nextPaths = pathMapping.getNextPathMappings().stream().map(PathMapping::getPath);

            return Stream.concat(Stream.of(pathMapping.getPath()), nextPaths).map(basePath::append);
        });

        if (selectedProperty.getProperty() instanceof AbstractRelation relation) {
            var nestedRequest = Optional.ofNullable(selectedProperty.getNestedRequest())
                    .orElseThrow(() ->
                            new OrchestrateException("Nested request not present for relation: " + relation.getName()));

            return sourcePaths.flatMap(sourcePath -> {
                var sourceRelTarget =
                        resolveSourceRelation(sourceTypeRef, sourcePath).getTarget();

                var nestedTypeMapping = modelMapping
                        .getObjectTypeMapping(nestedRequest.getObjectType(), sourceRelTarget)
                        .orElseThrow(() ->
                                new OrchestrateException("Type mapping not found for source root: " + sourceRelTarget));

                return resolveSourcePaths(nestedRequest, nestedTypeMapping, Path.fromProperties()).stream()
    ...
    private FetchOperation fetchSourceObject(
            ObjectTypeRef sourceTypeRef, Set<Path> sourcePaths, boolean isCollection, FilterMapper filterMapper) {
                ...
                .forEach((propertyName, nestedSourcePaths) -> {
                    ...
                    if (property instanceof InverseRelation inverseRelation) {
                        ...
                        nextOperations.add(NextOperation.builder()
                                ...
                                .delegateOperation(
                                        fetchSourceObject(originTypeRef, nestedSourcePaths, true, relFilterMapper))
                                .build());
                    ...
                    if (property instanceof Relation relation) {
                        ...
                        if (!filterMappings.isEmpty()) {
                            ...
                            if (!sourcePath.isLeaf()) {
                                throw new OrchestrateException(
                                        "Only leaf paths are (currently) supported for filter mapping: "
                                                + filterMapping.getProperty());
                            }
                            ...
                            if (targetProperty instanceof Attribute targetAttribute) {
                                ...
                                nextOperations.add(NextOperation.builder()
                                        .property(relation)
                                        .delegateOperation(fetchSourceObject(
                                                targetTypeRef, nestedSourcePaths, true, relFilterMapper))
                                        .build());
                            } else {
                                throw new OrchestrateException(
                                        "Filter property is not an attribute: " + targetProperty.getName());
                            }
                        ...
                        if (keyMapping != null) {
                            keyMapping.values().forEach(keyPath -> {
                                if (!keyPath.isLeaf()) {
                                    throw new OrchestrateException(
                                            "Only leaf paths are (currently) supported for key mapping: " + keyPath);
                                }
                            ...
                            if (property.getMultiplicity().isSingular()) {
                                selectedProperties.add(SelectedProperty.forProperty(
                                        property,
                                        ObjectRequest.builder(targetModel)
                                ...
                                selectedProperties.add(SelectedProperty.forProperty(
                                        property,
                                        CollectionRequest.builder(targetModel)
                                                .objectType(targetTypeRef)
                                                .selectedProperties(selectIdentity(targetTypeRef))
                                                .build()));
                            }
    ...
    private FilterMapper createFilterMapper(ObjectType sourceType, InverseRelation inverseRelation) {
        var keyMapping = inverseRelation.getOriginRelation().getKeyMapping();

        if (keyMapping != null) {
            // TODO: Composite keys
            var keyMappingEntry = keyMapping.entrySet().iterator().next();
    ...
    private Set<SelectedProperty> selectIdentity(ObjectTypeRef typeRef) {
        return modelMapping.getSourceType(typeRef).getIdentityProperties().stream()
                .map(SelectedProperty::forProperty)
                .collect(toSet());
    }
}
