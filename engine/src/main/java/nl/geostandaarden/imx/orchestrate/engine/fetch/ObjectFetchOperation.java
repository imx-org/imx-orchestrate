...
public final class ObjectFetchOperation extends AbstractFetchOperation {
    ...
    public Mono<ObjectResult> fetch(FetchInput input) {
        ...
        return source.getDataRepository().findOne(objectRequest).map(properties -> ObjectResult.builder()
                .type(objectType)
    ...
    public Flux<ObjectResult> fetchBatch(List<FetchInput> inputs) {
        ...
        if (!dataRepository.supportsBatchLoading(objectType)) {
            return Flux.fromIterable(inputs).flatMap(this::fetch);
        }

        var objectKeys = inputs.stream().map(FetchInput::getData).toList();
        ...
        return dataRepository.findBatch(batchRequest).map(properties -> ObjectResult.builder()
                .type(objectType)
                .properties(properties)
                .build());
    }
