...
public class NextOperation {
    ...
    public Publisher<ObjectResult> apply(Flux<ObjectResult> resultFlux) {
        ...
        return resultFlux.doOnComplete(dataLoader::dispatch).flatMapSequential(objectResult -> {
    ...
    private DataLoader<Map<String, Object>, ObjectResult> createDataLoader() {
        return DataLoaderFactory.newMappedDataLoader(objectKeys -> {
            var resultFlux = objectKeys.size() == 1
                    ? delegateOperation.execute(
                            FetchInput.newInput(objectKeys.iterator().next()))
                    : delegateOperation.executeBatch(
                            objectKeys.stream().map(FetchInput::newInput).toList());

            return resultFlux
                    .collectMap(result -> extractKey(result.getType(), result.getProperties()), identity())
                    .toFuture();
        });
    }
    ...
    private Publisher<ObjectResult> fetchCollection(ObjectResult objectResult) {
        ...
        if (!property.getMultiplicity().isSingular()) {
            return delegateOperation.execute(input).collectList().map(objectResults -> objectResult.toBuilder()
                    .property(
                            property.getName(),
                            CollectionResult.builder()
