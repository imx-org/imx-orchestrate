...
public class GraphQlAssert extends AbstractAssert<GraphQlAssert, String> {
    ...
    private void compareQueryNodes(
            List<GraphQlAssert.QueryNode> actualNodes, List<GraphQlAssert.QueryNode> expectedNodes, String field) {
        if (expectedNodes.size() != actualNodes.size()) {
            failWithMessage(
                    "Expected %s to have %d fields, but actually found %d.",
                    field, expectedNodes.size(), actualNodes.size());
        }
        ...
        while (iteratorActual.hasNext() && iteratorExpected.hasNext()) {
            ...
            if (!queryNodeActual.getName().equals(queryNodeExpected.getName())) {
                failWithMessage(
                        "Expected field %s to have field %s, but actually found %s.",
                        field, queryNodeExpected.getName(), queryNodeActual.getName());
            }
    ...
    private void compareArguments(QueryNode queryNodeActual, QueryNode queryNodeExpected) {
        if (queryNodeExpected.getArguments().size()
                != queryNodeActual.getArguments().size()) {
            failWithMessage(
                    "Expected field %s to have %d arguments, but actually found %d.",
                    queryNodeExpected.getName(),
                    queryNodeExpected.getArguments().size(),
                    queryNodeActual.getArguments().size());
        }
    ...
    private void compareArgument(Argument argumentActual, Argument argumentExpected, String field) {
        if (!argumentActual.getName().equals(argumentExpected.getName())) {
            failWithMessage(
                    "Expected field %s to have argument with name %s, but actually found %s.",
                    field, argumentExpected.getName(), argumentActual.getName());
        }
        ...
        if (!argumentActual
                .getValue()
                .toString()
                .equals(argumentExpected.getValue().toString())) {
            failWithMessage(
                    "Expected field %s to have argument with value %s, but actually found %s.",
                    field, argumentExpected.getValue(), argumentActual.getValue());
        }
    }
    ...
    private List<GraphQlAssert.QueryNode> getQueryNodes(String query) {
        ...
        var selectionSet =
                document.getDefinitionsOfType(OperationDefinition.class).get(0).getSelectionSet();
