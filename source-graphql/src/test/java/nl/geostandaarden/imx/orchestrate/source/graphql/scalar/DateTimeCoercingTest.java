...
class DateTimeCoercingTest {
    ...
    void parseLiteral_ReturnsDateTime_ForNowLiteral() {
        ...
        assertThat(dateTime.getYear(), equalTo(OffsetDateTime.now().getYear()));
        assertThat(dateTime.getMonth(), equalTo(OffsetDateTime.now().getMonth()));
        assertThat(dateTime.getDayOfMonth(), equalTo(OffsetDateTime.now().getDayOfMonth()));
    }

    @Test
    void parseLiteral_ReturnsDateTime_ForOffsetDateTime() {
        var dateTime = coercing.parseLiteral(
                        new StringValue(OffsetDateTime.now().toString()),
                        EMPTY_VARIABLES,
                        DEFAULT_CONTEXT,
                        DEFAULT_LOCALE)
                .get();
