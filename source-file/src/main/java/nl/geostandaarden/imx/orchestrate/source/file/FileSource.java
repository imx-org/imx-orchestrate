...
public final class FileSource implements Source {
    ...
    private static final PathMatcher matcher = FileSystems.getDefault().getPathMatcher("glob:**/*.json");
    ...
    public FileSource(Model model, Path folderPath) {
        ...
        try (var filePaths = Files.list(folderPath)) {
            filePaths.filter(Files::isRegularFile).filter(matcher::matches).forEach(this::loadFile);
        } catch (IOException e) {
    ...
    private void loadDocument(JsonNode data, ObjectType objectType) {
        if (data instanceof ArrayNode arrayNode) {
            arrayNode.elements().forEachRemaining(element -> loadObject(element, objectType));
            return;
        }
