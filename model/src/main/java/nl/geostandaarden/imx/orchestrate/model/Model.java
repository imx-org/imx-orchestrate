...
public final class Model {
    ...
    private Model(String alias, @Singular List<ObjectType> objectTypes) {
        ...
        this.objectTypeMap = objectTypes.stream().collect(toMap(ObjectType::getName, Function.identity()));
    }
    ...
    public Model replaceObjectType(ObjectType newObjectType) {
        var remainingTypes = objectTypeMap.values().stream()
                .filter(objectType -> !newObjectType.getName().equals(objectType.getName()));

        return toBuilder()
                .clearObjectTypes()
                .objectTypes(
                        Stream.concat(remainingTypes, Stream.of(newObjectType)).toList())
                .build();
    }
}
