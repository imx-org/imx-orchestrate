...
public final class ModelMapping {
    ...
    public ModelMapping(
            Model targetModel,
            @Singular Set<Model> sourceModels,
            @Singular Set<SourceRelation> sourceRelations,
            @Singular Map<String, List<ObjectTypeMapping>> objectTypeMappings,
        ...
        this.sourceModelMap = this.sourceModels.stream().collect(toMap(Model::getAlias, Function.identity()));

        this.objectTypeMappings = objectTypeMappings;
        this.lineageNameMapping = Optional.ofNullable(lineageNameMapping).orElse(Map.of());
    }
    ...
    private Set<Model> addSourceRelations(Set<Model> models) {
        ...
        var mutableSourceModelMap = models.stream().collect(toMap(Model::getAlias, Function.identity()));
        ...
        sourceRelations.forEach(sourceRelation -> {
            ...
            mutableSourceModelMap.put(
                    sourceModelAlias,
                    sourceModel.replaceObjectType(
                            sourceModel.getObjectType(sourceType).appendProperty(sourceRelation.getProperty())));
        });
    ...
    private Model resolveInverseRelations(Model model) {
        return model.getObjectTypes().stream()
                .flatMap(objectType -> objectType.getProperties(Relation.class).stream()
                        .flatMap(relation -> {
    ...
    private Set<Model> resolveInverseRelations(Set<Model> models) {
        ...
        models.forEach(
                model -> model.getObjectTypes().forEach(objectType -> objectType.getProperties(Relation.class).stream()
                        ...
                        .forEach(relation -> {
                            ...
                            var relTargetModel = Optional.ofNullable(relTarget.getModelAlias())
                                    ...
                                    .orElseThrow(() -> new ModelException(
                                            "Source model not found for relation target: " + relation));
                            ...
                            mutableSourceModelMap.put(
                                    relTargetModel.getAlias(),
                                    relTargetModel.replaceObjectType(relTargetModel
                                            .getObjectType(relTarget)
    ...
    public ObjectType getSourceType(ObjectTypeRef sourceTypeRef) {
        ...
        return getSourceModel(sourceTypeRef.getModelAlias()).getObjectType(sourceTypeRef.getName());
    }
    ...
    public Optional<ObjectTypeMapping> getObjectTypeMapping(ObjectType targetType, ObjectType sourceRoot) {
        ...
        return getObjectTypeMappings(targetType).stream()
                .filter(typeMapping -> typeMapping.getSourceRoot().getName().equals(sourceRoot.getName()))
                .findFirst();
    }

    public Optional<ObjectTypeMapping> getObjectTypeMapping(ObjectType targetType, ObjectTypeRef sourceRootRef) {
        return getObjectTypeMappings(targetType).stream()
                .filter(typeMapping -> typeMapping.getSourceRoot().getName().equals(sourceRootRef.getName()))
                .findFirst();
    }
}
